...
| **`Alt+Enter`** | **lock/release** —— 給遠端也在跑 sshu 的巢狀場景。locked 的格子把每一個鍵都穿透過去,內層 sshu 的和絃就全部活了;這個選單在鏈上的每一層都會開、各自說出自己的狀態,而 Alt+Enter 是 lock 唯一吞不掉的鍵。底下有鏈的時候,選單另有兩列**沒有熱鍵**的整鏈動作:全部 zoom max + 除最內層外全鎖(最內層留著,和絃才有地方去),以及反向的還原 |
