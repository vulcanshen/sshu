...
tab 用 **`Alt+P` / `Alt+F` / `Alt+S`** 和絃切換 —— 是和絃,所以遠端拿著鍵盤時照樣有效 —— 而裸數字 `1`–`9` 全部用來直達**當前 tab** 的 panel。
...
**`[Alt-F]ileTransfer`** —— 兩個各自獨立的檔案系統並排,1:1。`local` 開在你啟動 sshu 的目錄,所以 `cd ~/release && sshu` 一進去就在那批東西上。任一側可以是本機或某台已存的 host,而且**兩側都可以是遠端**,所以上傳、下載、遠端對遠端是同一個操作而不是三個。標記你要的、跨到另一邊、送出。`/` 搜尋的是**整棵子樹**,不是螢幕上那個目錄;`v` 不用抓下來就能讀,`e` 直接用你自己的編輯器開。

**`[Alt-S]sh`** —— 一個**活終端機的網格**,每一格都是自己 PTY 上的真 `ssh`。session 清單上 `Tab` 切換格子上下網格、`Enter` 顯示並把鍵盤交過去、`Alt+1`–`9` 在格子之間跳、`Alt+Esc` 把鍵盤收回來。layout 條紋決定排列:水平、垂直,或自訂的欄 × 列。
...
```
 tab       Alt+P / Alt+F / Alt+S(pty 裡也有效)
...
### `[Alt-F]ileTransfer` —— 小寫是游標那一列,大寫是整個 panel
...
| `t` | 傳到另一側的當前目錄 |
| `x` | 刪掉(先問) |
| `/` | **搜尋整棵子樹** —— `Enter` 帶你到結果所在的位置、游標停在它上面,`m` / `t` / `v` / `e` / `x` 從那裡全部能用 |
...
| **`Alt+1`–`9`** | 跳到第 N 格 —— 從清單、layout 條紋或另一格都行 |
| **`Alt+Esc`** | **從遠端手上把鍵盤收回來** —— 回到清單,側欄回來 |

layout 條紋(`2`)決定網格排列:`h`/`l` 在**水平 / 垂直 / 自訂**之間走、走到就生效;在自訂上按 `Enter` 問欄 × 列(兩個 1–9 的數字)。列上寫的是 `<user>@<host>` 加右邊界的 port,開頭是顯示欄 —— 有格子的是 monitor glyph、沒有的是劃線的那個。
...
- **並存 ssh session 的網格** —— 每一個都是 embedded PTY 裡的真 `ssh`,同時上畫面幾個都行,水平、垂直或自訂欄 × 列排列。每一格的遠端只在尺寸真的變了才收到通知。結束的 session 立刻離開網格並放掉模擬器;鍵盤絕不會默默落進另一台遠端。
