...
`[1]` 空表格必須同時揭露 `[A]` 與 `Space`;`[2]` 沒選 host 的那一側寫
`Press [S] to select a host, or local`。一個新使用者面對空 panel 無路可走,X 直接
掉(§A.0)。
...
| 全部 | `r` · `S` · `C` · `P` | Rename / Select host / Clear marks / Progress |
| `[4]` `[6]` | `Enter` · `Esc` | 進目錄 / **先退搜尋、再退上一層** |
| `[4]` `[6]` | `m` · `/` · `N` | 標記(可再按取消)/ 搜尋整棵子樹 / 新目錄 |
| `[5]` `[7]` | `m` | 取消這一個標記(**同一個 `m`**) |
