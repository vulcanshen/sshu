...
### 4.5 文字輸入 surface —— 通用 §4.5 在 sshu 的落點

form / file picker / Rename 輸入框裡,**空白就是空白、問號就是問號**,判斷收在
一個 `m.textFloat()` 裡。

這裡原本寫的是「§A.0.K 入口鍵規則的**唯一例外**」。**它不再是例外** —— 原則新增了
通用 §4.5:使用者正在輸入文字時,所有會產生字元的鍵都進輸入框、不觸發任何動作,
只留 `Esc`(取消)、`Enter`(送出)、`Tab` / 方向鍵(欄位移動)。全部屏蔽會讓
輸入框變成出不去的陷阱,所以那三類留著。

那條會升成通則、而不是留給各 app 自己處理,是因為它**不是場景問題**:core-key
裡有兩個本身就是可列印字元(`Space` 與 `?`),不屏蔽就打不出含空白的檔名。三個
app 都做了同一件事,只是收斂的位置不同 —— sshu 在 `m.textFloat()`,filu 靠
`inputPopup.isActive()` 的前置守衛,kbu 在 picker 的 keyboard search。
