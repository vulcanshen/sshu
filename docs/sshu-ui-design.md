...
| **`~/.ssh/config` 面板**(表格 + 明細 + 動態表單 + CRUD) | 已落地(§11.39) | `ui/sshcfglist.go` `ui/sshcfgkeys.go` `ui/sshcfgform.go` |
| **外科手術式寫回**(只動被編輯的那幾行;註解 / `Match` / 未知關鍵字原樣保留) | 已落地 | `store/sshconfig.go` |
| **存檔前比對磁碟**(逐檔;別人先寫就拒絕,並把他那份端上畫面) | 已落地 | `store/sshconfig.go SaveSSHConfig` |
| **`Include` 跟進去**(就地展開、glob 排序、相對於 `~/.ssh`、16 層上限、循環會停);被 include 的區塊照樣可編,寫回它自己的檔 | 已落地(§11.42) | `store/sshconfig.go scan` `follow` |
...
### 11.25 zoom —— `Alt+Z` 進去,`Alt+Esc` 一次剝一層

> 本節記錄 zoom 的原始設計。之後有兩次推翻:鍵位由 `Alt+Enter` 換成 `Alt+Z`
> (§11.43),階段由一階變三階、並多出一個「滿版」階段(§11.47)。被推翻的
> 段落就地標註,原本的理由保留 —— 它們在當時是對的,知道它們何時停止成立
> 比刪掉有用。
...
> **§11.47 起這張表多一列**:滿版是使用者疊上去的第二層,所以先退回網格 zoom、
> 再退出 zoom、最後才收回鍵盤。規則沒變,只是層數變了。

#### ~~只有一格時,這個鍵是遠端的~~(§11.47 起不再成立)

一格已經填滿網格,zoom 什麼都不會改變。而**一個按下去看不出變化的鍵,第一次
按就會被讀成壞掉**(同 §11.19 的判斷)。所以 `canZoom()` 為 false 時 sshu
**不攔截**它 —— 原封不動送給遠端,跟任何其他和絃一樣。footer 那格也跟著不出現。

> 這條的前提是「網格區就是 zoom 能拿到的全部」。§11.47 讓 zoom 能再拿走 chrome,
> 而 chrome **永遠在**,所以「這一格已經沒東西可拿」的網格不存在了 —— 判斷式從
> `canZoom()` 這個總開關,降級成 `nextZoom()` 裡「網格階」自己的條件。
...
> **滿版階段是例外**(§11.47):那裡連 chrome 都不畫,「巢狀 sshu 裡的一個
> shell」和「一個裸 shell」變成同一張畫面 —— 這才是「有別的狀態長得像它」,
> 所以那一階才需要 badge。這一階不需要,理由沒變。

footer 仍然變:它說的是**現在按 `Alt+Esc` 會拿掉哪一層**(`leave zoom` /
`leave pty`),以及 `Alt+Z` 的下一次按會做什麼(§11.47 之後有三個標籤:
`zoom` / `full screen` / `unzoom`)。
...
### 11.39 `~/.ssh/config` —— 一個不是 sshu 的檔案

#### 起點是一個被推翻的前提

使用者問的是:「我們又不是去執行 ssh 指令開 pty, 所以這個 config 檔對我們有用處嗎?」

反過來。`internal/ui/session.go:82` 就是 `exec.Command(sshBinary, args...)`,
而那個檔案自己的註解把理由寫死在那裡,第一項就是它:

> sshu shells out to the real ssh rather than speaking the protocol itself, so
> **ssh_config**, ProxyJump, agent forwarding, host-key handling and everything
> else the user already relies on keeps working.

所以 `~/.ssh/config` **不是**「要不要多管一個檔案」的問題 —— 它**現在就在**決定
tab `[3]` 每一條連線的行為,只是完全看不見。

#### 真正的重點是一個不對稱

| | 走什麼 | 吃 `~/.ssh/config` |
|---|---|---|
| `[3]` SSH pty | `exec.Command("ssh", …)` | **吃**,完整地吃 |
| `[2]`/`[4]` SFTP | `golang.org/x/crypto/ssh` 自己講協定 | **完全不吃** |

sshu 傳給 ssh 的只有 `-p`、`-o ConnectTimeout=`、privatekey 時的 `-i` +
`IdentitiesOnly=yes`,和 `user@host`。命令列優先,所以 **Port / User /
IdentityFile 是 sshu 說了算**;剩下的 —— `HostName`、`ProxyJump`、
`ForwardAgent`、`StrictHostKeyChecking`、`SetEnv`、`ServerAliveInterval` ——
**全部是 config 說了算,sshu 一個字都沒傳**。

最尖的是 `HostName`:一個 sshu host 的 `host` 欄若填的是 config 裡的 alias,
`[3]` 會被 config 換成真位址而連得上,`[2]`/`[4]` 拿 alias 直接丟給 DNS 而連不上。
同一台 host、兩個分頁、不同結果,而 UI 目前沒有任何地方解釋。

#### 唯讀 vs. 可寫 —— 使用者的裁決

我提出的顧慮**沒有被推翻,是被接受後仍然選擇了 CRUD**,所以兩邊都記下來:

- sshu 的 host 記錄本來就有 host / port / user / identity,和 config 的
  `HostName` / `Port` / `User` / `IdentityFile` **職責重疊**。多一個能寫的地方
  就是多一個能互相矛盾的地方。
- 純粹「編一個文字檔」這件事,`$EDITOR` 做得比表單好,而 `ui/editorcmd.go` 已經在了。
- 就算編好了,也只對一半的 sshu 有效(SFTP 那半不吃)。

**使用者裁決:完整 CRUD。** 立場是「sshu 是我管 ssh 的地方,那 ssh 的設定也該在
這裡管」。**被否決的:唯讀揭露面板**(只回答「這台其實是怎麼連的」)、**只在 host
明細裡加一段揭露**。
...
`parse → struct → marshal` 會是三分之一的程式碼,而且會**默默吃掉**註解、`Match`
區塊、第一個 `Host` 之前的全域選項、以及每一個 sshu 不認識的關鍵字 —— 在一個
git、rsync、scp 和機器上一半的編輯器也在讀的檔案裡。所以 parser **留著檔案自己的
每一行**,寫回時只重建動到的那個區間;沒被編輯到的那些行,`setValue` 會原字串
回傳,連分隔符與行尾空白都不動。

有一個測試專門釘住這件事:改一個值 → **整份檔案只有一行不同**。

#### 位置是身分,不是名字

同一個 `Host` pattern 出現兩次在 ssh 裡**合法且有意義** —— 每個關鍵字取第一次
出現的值,所以兩個 `Host prod` 區塊都會生效。這是 §11.38 的**反面**:那裡重複的
name 進來就去重(因為第二列在別的地方全部不可達),這裡重複的區塊**一列都不能少**。

代價是浮層與確認框沒有名字可以拿來指認,所以 `detailPopup` 與 `confirmPopup` 各多
一個 `at int`。**被否決:把 index 塞進既有的 `target string`。** 用十進位字串當
identity 再 `Atoi` 回來,是把型別錯誤藏進字串裡。
...
# production      ← 這是 prod 的標籤
Host prod
```

刪掉 `Host a` 不能把 `# production` 帶走;刪掉 `prod` 必須帶走。所以每個區塊
**吐出**它尾端的註解、**吃下**正上方**沒有空行隔開**的註解 —— 那個空行就是「檔案
自己的抬頭」與「這一塊的標籤」之間的分界。

#### 表單的欄位數不能固定

ssh 有大約一百個關鍵字。五個常見到值得一個永久欄位,其餘是這個區塊剛好用到什麼。

實證來自使用者自己的 `~/.ssh/config`:35 行、5 個 `Host` 區塊,`AddKeysToAgent`×3、
`StrictHostKeyChecking`×2、`UserKnownHostsFile`、`UseKeychain`、`SetEnv` —— 但只有
2 個 `HostName` / 2 個 `User`。也就是說有 `Host *` 這種區塊,**它做的每一件事都在
那五欄之外**。固定五欄的表單打開它會是五個空格子,而那個區塊真正在做的事一個都碰不到。

所以:五個固定欄 → 分隔線 → 這個區塊現有的每個關鍵字各一列(標籤用**檔案自己的
拼法**)→ 一列 `+ add option`。

**每一列仍然是單行文字欄**,這是關鍵:多行編輯器的 Enter 要插入換行,會跟 §11.34
「form 的 Enter 只問一個問題」正面衝突,等於為它另開一個浮層類別。**被否決的第三個
方案就是那個**(直接編區塊的原始文字)。

- **`+ add option` 的 Enter = 「加這一個」**,不是提交。半打完的選項不是「填完了」,
  所以 Enter 把它收成一列、游標落在新那列上 —— 這仍然是 §11.34 的同一個問題。
- **空值 = 刪掉那一行。** ssh 沒有「有關鍵字沒有引數」這種東西,所以「把欄位清空」
  是唯一可能表示「拿掉這行」的手勢,而且是人本來就會伸手去做的那個。
- **`Host` / `Match` 不能當選項關鍵字。** 兩者都會在那一行**結束這個區塊**,上面
  一半的選項會默默歸給一個沒人想建立的區塊。這是唯一一條純粹保護檔案的驗證。
- **已經有那個關鍵字就跳過去,不開第二列。** ssh 只讀第一個,第二列會是一行看起來
  有用、實際沒有作用的東西。
- **重複的關鍵字不被丟掉。** 一個區塊裡若真的有兩行 `User`,第一行綁到固定欄、
  第二行成為一個獨立的選項列。第二行是死的,但它是**使用者的**死行 —— 載入時默默
  丟掉它,等於下一次存檔就從他檔案裡刪了一行。

#### 驗證少得刻意

ssh 自己會驗它的關鍵字。sshu 因為「不認得」就拒絕一個值,會讓這個面板比一個
編輯器還糟。剩下的兩條是會產生 **ssh 根本讀不了的區塊**的那兩條:pattern 必填、
Port 若有值必須是 1-65535。

#### ~~Include:只計數,不跟進~~ —— 這個決定後來被推翻了(見 §11.42)

> **當初的決定(留著,因為它的理由沒有錯,只是不夠):**
>
> 只 parse `~/.ssh/config` 本體。`Include` 那行原樣保留,並且**說出來**:狀態列寫
> `2/5 blocks · 1 include not listed`。不說的話,畫面上這份清單會被當成全部。
>
> **被否決:跟著讀進來一起列但唯讀**(多一種「看得到卻動不了」的列)、**完全跟進去**
> (寫回從一個檔案擴大成一組檔案、還可能是 glob,而且 `[A]` 得先問「這筆寫進哪個檔」)。

推翻的理由在 §11.42:那三條顧慮**全部是關於寫**,沒有一條是關於讀 —— 而做完
§11.41 的聯集之後,「這台實際會套用什麼」底下掛一句「有東西我沒讀」,是一個承認
自己答錯的答案。

順帶一個 parser 的正確性註腳,這條**沒有**變:**`Include` 若寫在某個 `Host` 區塊
底下,它就屬於那個區塊**(ssh 的作用域就是這樣)。

#### 這個檔案有別的編輯者

`hosts.yaml` 只有 sshu 在寫;`~/.ssh/config` 有 vim、VS Code、dotfiles repo。而
sshu 是**整份重寫**的,所以存檔前先重讀比對,不一樣就**拒絕**。

拒絕時把**磁碟上那份**一起回傳,面板直接換成它 —— 訊息落地的時候畫面上已經是實話。
否則會是「清單顯示改動成功」旁邊掛一則「其實沒成功」的 toast。錯誤訊息用
`FoldHome(path)`,因為窄 toast 裡要活下來的是**原因**,不是路徑。

**權限保留原檔的,不強制 0600。** ssh 只在**別人可寫**時才抗議;一個使用者刻意留成
0644 的檔案,不是 sshu 該替他改窄的東西。新建的檔案才給 0600。

#### 這一版沒做的

**不接連線、不接 Credentials。** 使用者明說「他跟 Credentials 的關係我們可以再聊」,
而「刪掉 `Host prod` 時警告有哪些 sshu host 會受影響」正是那場對話 —— 它需要 ssh
pattern 比對(`*`、`?`、`!` 否定),是一個獨立的決定。

#### mutation 清單(23 個,全數被抓)

store:區塊尾端的註解不吐出來 / 正上方的註解不吃進來 / `Set` 丟掉區塊內的註解與空行 /
`Set` 把每個選項都當新的(等於整塊重寫)/ `Match` 不再結束區塊 / `Include` 不回報 /
重複的 `Host` 被去重 / 存檔不比對磁碟 / 存檔強制 0600 / `Add` 插到檔案最前面 /
空值的選項照樣寫一行 / 清空欄位不再刪行 / 沒有 pattern 的區塊被接受。

ui:狀態列不再揭露 Include / 拿掉 `Opts` 欄(當時還有這一欄,見 §11.42 追記) / 明細不給 offer / 表單疊在明細上而不是取代它 /
存檔被拒時面板留著自己那份 / 確認框不再數選項 / 表單只給五個固定欄 / `Host` 可以當
選項關鍵字 / 已有的關鍵字會開第二列 / 表單不捲(長區塊的尾巴按得到看不到)。

最後那個的測試帶一條 **precondition 斷言** —— 欄位若少到不會溢出,那個測試就什麼都
沒證明,所以它會先 `Fatalf` 說「setup 沒有製造出那個情境」。

### 11.40 `~/.ssh/known_hosts` —— 有牙齒的那一個

#### 為什麼是它

跟 §11.39 同一個理由,但更直接:`internal/remote/sftp.go` **已經在驗證它**,而且
**金鑰變了就直接拒絕連線**,不給 yes/no —— 那是刻意的,把它變成一個問句只會教會
人一律按 yes。

問題是碰到那個拒絕之後**沒有出路**。伺服器重灌是家常便飯,而目前唯一的解法是開
另一個終端機跑 `ssh-keygen -R`。這個面板就是那條出路。

#### 這個檔案比 config 更禁不起重寫

實測使用者自己的 `~/.ssh/known_hosts`(只數結構、沒讀主機名也沒讀金鑰):
...
**註解比條目還多。** 「parse 成結構再吐回去」在這裡不是有風險,是會直接刪掉一百多行
別人的東西。所以沿用 §11.39 那套:留著檔案自己的每一行,**一筆 = 一行**,改名只
splice 那一行的第一個欄位,其餘 byte 不動。

#### 一條跟 Config **相反**的規則

**刪除一筆條目,不帶走它上面的註解。**

`~/.ssh/config` 的 `# production` 是標在 `Host prod` 這個**區塊**上的,所以刪區塊
要帶走它。known_hosts 的 `# work` 標的是**一整串**條目 —— 刪掉其中一列就把整串的
標題也刪掉,是比留下孤兒更糟的錯。同樣是「註解歸屬」的問題,答案相反,因為被標的
東西不同。

#### CRUD 在這裡是不對稱的

`[X]` 是日常操作(那就是 `ssh-keygen -R`)。`[A]` 和 `[E]` 不是,而承認這件事才做得
對:

**`[A]` 去問那台機器,不叫你打 base64。** 一個要求輸入 68 個字元 base64 的表單,是
沒有人會用的表單 —— 而且那串字你本來就得從別的地方拿到,那個「別的地方」就是伺服器
本身。所以 `[A]` 做 `ssh-keyscan` 做的事、也是 ssh 第一次連線時做的事:把連線開到
**看得到金鑰為止就停**(在認證之前中止 —— 再往前一步就是把憑證遞給一台還沒決定要
信任的機器),然後把指紋放上來讓你決定。
...
**沒有 `[D]uplicate`,而這個缺席本身是設計。** 複製一筆 host key 的意思是「同一把
金鑰再信任一個名字」,而那正是名稱欄位的逗號清單在做的事 —— 一行,不是兩行。
**一個沒有意義的動作比缺一個動作更糟**:它佔掉一個字母,並且教錯這個檔案的模型。

#### 有些列說不出自己是誰

`|1|salt|hash` 是 HMAC。**主機名讀不回來** —— 那正是把它 hash 掉的目的。所以那一格
寫 `(hashed)`,狀態列補一句 `1 hashed`:不說的話,那幾格會被當成畫面壞掉。分辨那些
列靠的是指紋欄。

`@revoked` 把整列的意思**反過來**。所以 marker 跟在名字前面、而且吃 warning 色 ——
一列在實際上是「明確不可信」時卻讀起來像被信任的,是這個面板唯一不能犯的錯。

#### 指紋算兩次,而且必須一致

`store` 從存下來的 base64 算(`SHA256:` + 未補 padding 的 base64 of SHA-256,就是
`ssh.FingerprintSHA256` 在做的事),`remote` 則從握手拿到的 `ssh.PublicKey` 算。
兩段獨立的程式碼算同一件事 —— 使用者在信任前看到的那一串,跟事後那一列顯示的那一串,
必須是同一串,否則那一列看起來就不是他答應過的東西。有一個測試專門釘住這件事。

#### 存檔前比對磁碟,在這裡不是保險是常態

`hosts.yaml` 只有 sshu 在寫。`known_hosts` **每次 ssh 第一次遇到一台新機器就會被
追加一行**。所以「別人在 sshu 開著的時候寫了」是每天都會發生的事,不是罕見情況 ——
被拒絕時把磁碟上那份端上畫面,訊息落地時畫面上已經是實話。

權限一樣保留原檔的;新建才給 0600。

#### 被否決的做法

- **完整手打(Add / Edit 都能貼 base64)。** 工量最小、跟 Config 面板最一致 ——
  但那串 base64 你得先從別處弄到手,而「別處」就是伺服器。
- **只做 Delete + 明細。** 誠實反映了實際使用比例,但使用者裁定要完整 CRUD,而
  「去抓」讓 Add 變成一個真的有人會按的鍵。
- **把 index 塞進 `target string`。** 同 §11.39:`inputPopup` 也跟著長出 `at int`。

#### mutation 清單(26 個,全數被抓)

store:`SetHosts` 整行重建而不是 splice 欄位 / 刪除連上面的註解一起帶走 / 欄位不足
的行被當成條目 / marker 被當成主機名 / hashed 不被辨識 / 指紋補了 padding / 壞掉的
base64 也給指紋 / 名字裡的空白被接受 / 沒有金鑰的條目被接受 / 存檔不比對磁碟 /
存檔強制 0600 / 寫入失敗回傳沒落地的那份。

remote:抓回來的不是 wire format / 握手什麼都沒收集到。

ui:hashed 列印出 HMAC / marker 從列上消失 / 狀態列不再數 hashed / 金鑰型別不縮寫
(比欄位還寬)/ 非預設 port 寫成裸名字 / 抓到金鑰直接寫入不問 / 確認框不顯示指紋 /
刪除確認不顯示指紋 / 改名框從空白開始 / 存檔被拒時面板留著自己那份 / 抓取失敗把
表單關掉 / 長出一個沒有意義的 `[D]uplicate`。

前兩個一開始**活下來了**,而且都是**fixture 沒有製造出那個情境**:我的樣本每一行都
是單空格、沒有 marker,所以「整行重建」剛好產生一模一樣的字串;樣本裡也沒有任何
「不是註解、也不是合法條目」的殘行。補上一行對齊過的 `@cert-authority` 和一行殘行
之後兩個都被抓到。**跟 §11.29 那次同一種形態**(見 §11.39 最後一段)。

### 11.41 兩份 host 清單 —— 誰蓋掉誰

#### 使用者的質疑,以及它是對的

> 「但一個是 sshu 的 hosts 設定,一個是 ssh 的 config,一個是 Hosts 紀錄,一個是
> ssh 連線的 config,為什麼會衝突?」

分層是對的。我先前寫的「職責重疊 → 多一個能矛盾的地方」講得太含糊,精確的說法是:

**`hosts.yaml` 的欄位不是跟 ssh config 並排的另一份設定 —— 它們會變成 ssh 的
命令列參數,而命令列參數的優先權高於 `~/.ssh/config`。**

`buildSSHCmd` 組出來的是:

```
ssh -p <hosts.yaml 的 port> -o ConnectTimeout=N [-i <key> -o IdentitiesOnly=yes] <user>@<host>
```

#### 一個會真的連錯地方的例子

`~/.ssh/config` 有 `Host gw / HostName 198.51.100.10 / Port 2222`,
`hosts.yaml` 有一台 `host: gw`、port 欄沒動(載入時補成 22)。

| | 誰說了算 | 結果 |
|---|---|---|
| 連哪台機器 | **config** —— sshu 只傳了 `gw` 這個 token,ssh 自己去查 `HostName` | `198.51.100.10` |
| 連哪個 port | **sshu** —— `-p 22` 蓋掉 `Port 2222` | `22` |

連到 **198.51.100.10:22**,一半來自 config、一半來自 sshu。

規則是:**sshu 有傳的它贏**(port、user、privatekey 時的 identity、
ConnectTimeout),**sshu 沒傳的 config 贏**(`HostName`、`ProxyJump`、
`ForwardAgent`、`StrictHostKeyChecking`、`SetEnv`…)。而 `HostName` 落在第二組,
它決定的是你到底連上哪一台 —— 所以分界線是「**config 決定哪台機器,sshu 決定哪個
port 和哪個使用者**」,一條沒人寫下來、只是「`buildSSHCmd` 剛好傳了哪幾個 flag」的
副產品。

**只有 `host` 欄填的是 config 裡的 alias 時,兩層才會纏在一起。** 填真實位址的
host 完全不會踩到 —— 那時兩個檔案確實各管各的,而那正是 sshu 去啟動真 `ssh` 而不是
自己講協定的好處。

#### 第二個修正:是聯集,不是「命中哪一條」

> 「所以應該是吃到所有 `~/.ssh/config` 中設定的『聯集』,顯示這個吧?這才跟 ssh
> 行為相同?」

也是對的,而且我的框架整個錯了。ssh **不挑區塊** —— 它掃過整個檔案,**每個關鍵字
取第一次出現的值**,跨所有命中的區塊。`ssh_config(5)`:
...
所以「這台命中哪一個區塊」是個問錯的問題:一台 host 通常同時命中好幾個,並從每一個
拿走一點東西。

#### 落到 host 明細上

**一個貢獻者一個 section,照檔案順序**,所以最上面那個 section 就是值贏了的那個 ——
順序本身就是優先權,不必再解釋一次:
...
- **被 shadow 的值不列出來。** `Host *` 的 `Port 22` 在具體區塊已經給過 `Port` 之後
  就不會出現 —— 列出來等於列一個 ssh 不會用的值。
- **值真的不同才標紅。** `User vulcan` 被 sshu 送的 `vulcan@` 蓋掉,兩邊同一個答案,
  **那不是覆蓋**。原本每一台都會亮一條紅線,是在喊狼來了。(實機跑出來才看到的。)
- **什麼都沒命中就整段不出現。** 填真實位址的 host 通常只遇到 `Host *`;一個都沒遇到
  的就什麼都不畫。
- **必須說出自己漏了什麼。** sshu 只 parse 本體:`Include` 進來的沒讀、`Match` 是
  條件式的沒套用。一段標題寫著「這台實際會套用什麼」卻默默漏掉這兩種來源,比沒有這
  一段更糟 —— 所以 `not shown` 那一節在有東西沒算進去時是強制的。

#### 反向:刪區塊時警告哪些 sshu host 會受影響

兩個數字,嚴重程度不同:

- **有幾台的 `HostName` 來自這一塊** → 「1 sshu host resolves through it and will
  not connect.」刪掉就連不上了。
- 否則有幾台命中它 → 「2 sshu hosts inherit options from it.」只是少繼承一些東西,
  這是 `Host *` 會產生的。

**credential 不列入,而這是使用者的裁定:**

> 「跟 credential 無關,credential 是 sshu 特別的儲存方式,和 `~/.ssh/config` 是
> 獨立的,所以只需要警告有哪些 sshu hosts 使用就好」

對的 —— credential 供給的是 sshu host 的 user + auth,裡面沒有任何東西會抵達
`~/.ssh/config`,所以它對一個 Host 區塊消失無話可說。

#### 為什麼自己寫 pattern 比對器

`filepath.Match` 錯兩件事:**沒有 `!` 否定**,而且**不讓 `*` 跨越 `/`** —— 主機名有
權利含有斜線。ssh 的規則是:pattern 以空白分隔、`*` 任意長、`?` 剛好一個、比對
不分大小寫,而 `!` 開頭的 pattern **一票否決整個區塊**,不論有幾個正向 pattern 也
命中了。實作是迭代式的 glob(不是遞迴),所以 `*a*a*a*` 不會爆炸。有一個測試直接
斷言 `filepath.Match` 在斜線那題上**不同意** —— 如果哪天它同意了,這個比對器存在的
理由就變了,那條斷言會說出來。

#### mutation 清單(12 個)

否定不再否決 / 比對變成大小寫敏感 / `*` 不跨斜線 / 聯集取最後一個值 / **`Effective`
只吃第一個命中的區塊**(就是使用者糾正我的那個錯誤模型)/ `Match` 不計數 / 覆蓋的列
不再說被蓋掉 / 不再揭露漏了什麼 / 什麼都沒命中也畫出那一段 / 每台命中的都算成「會
連不上」/ 刪除確認不再數 sshu host / 一個選項時動詞退回複數。

第九個一開始**活下來**,又是**fixture 沒製造出那個情境**:那個測試的 config 沒有
`Include` 也沒有 `Match`,所以拿掉守衛之後「未列出」那一節本來就是空的。加一個
`Include` 進去就踩得到了 —— 這一輪第三次踩到同一種。

### 11.42 Include 跟進去 —— 把「我沒讀」那一行拿掉

#### 推翻 §11.39 的一個決定

> 「如果有 include, 那是否要把 include 的項目都 load 進來?」

§11.39 當初決定「只管本體、但明講有東西沒列出」。做完 §11.41 的聯集之後,那個決定
就站不住了:一段標題寫著「這台實際會套用什麼」,底下掛著一句「有東西我沒讀」——
那是一個承認自己答錯的答案,而且那句話會出現在**每一台**命中任何東西的 host 上。

回頭看,當初那三條顧慮**全部是關於寫**:寫回從一個檔案擴大成一組、可能是 glob、
`[A]` 得先決定寫進哪個檔。**沒有一條是關於讀。** 讀進來永遠是對的;真正要決定的是
「讀進來之後,那些區塊能不能改」。使用者裁定:**能改**。

#### ssh 的 Include 沒有一條是簡單的

| | |
|---|---|
| 一個指令可以有**多個路徑** | 空白分隔 |
| 路徑可以是 **glob** | 而且**順序決定誰贏**,所以要排序 |
| 相對路徑**相對於 `~/.ssh/`** | 不是 cwd,也不是「include 它的那個檔」的目錄 |
| **可以巢狀** | OpenSSH 自己有 16 層上限 —— sshu 用同一個數字,那是同意它而不是自己發明規則 |
| 可以寫在某個 `Host` 區塊**裡面** | 那它是條件式的 |
| 內容是**在那一行的位置展開**的 | 所以 first-wins 的順序要跟著 |

最後一條是實作上最尖的:**遞迴必須把「現在在哪個區塊」存起來再還原**。

```
Host prod
  HostName p.example.com
  Include conf.d/extra      ← 展開一整個檔案
  Port 2222                 ← 這一行還是 prod 的
```

實作上不必特別做什麼 —— `cur` 是 `scan()` 的**區域變數**,遞迴天生就存了又還原。
而被 include 的檔案自己是從 `cur = -1` 開始的,所以它開頭那些沒有 `Host` 罩著的裸
選項不屬於任何人 —— 跟主檔第一個 `Host` 之前那些的待遇一樣,這是**既有行為的延伸,
不是新規則**。

#### 一筆資料現在有兩個座標

`SSHConfigFile` 從「一個檔案的行」變成「一棵樹的檔案」,`SSHBlock` 多一個所屬檔案。
於是:

- **編輯只動那個區塊自己的檔案。** 外科手術式寫回不變,只是現在要先問「哪個檔」。
- **存檔只寫改過的檔案。** 一棵樹裡多數檔案沒被碰過,而把它們全部重寫等於在別人的
  dotfiles repo 裡製造一份沒人要求的 diff。有測試用 mtime 釘住。
- **比對磁碟逐檔做。** sshu 重寫的是**整個檔案**,所以被別人動過的那一個必須被拒絕,
  而且不能因為別的檔案剛好沒變就跳過檢查。
- **`[A]` 一律寫進主檔。** 一棵 include 樹沒有「顯然」該放新東西的地方,而用猜的
  會把一台 host 放進一個跟團隊共用的檔案裡。
- **確認框點名檔案。** `[X]` 現在可能改寫一個使用者沒開過、甚至不記得自己有的檔案。
  一個寫著「This rewrites ~/.ssh/config.」卻其實在動 `config.d/work` 的確認框,
  是在最要緊的那一刻說謊。

#### 兩個顯示上的決定

**`File` 欄只在**檔案多於一個時出現。一欄在每一列都寫著 `~/.ssh/config` 的欄位,是
一欄什麼都沒說的欄位。

**追記(使用者):**「Opts 好像沒有必要, File 欄位內容應該給絕對路徑,不然相同檔名就
都一樣」—— 兩條都對,而且第二條指出了一個真的 bug:我原本顯示 **base name**,而
`config.d/work/hosts` 和 `config.d/home/hosts` 會渲染成同一列,那正是這一欄存在的
唯一理由。

所以現在:**`File` 是彈性欄、放完整路徑、而且從左邊截斷。** 路徑有多長就是多長,固定
的窄欄位會剛好切掉能區分它們的那個尾巴;`truncateHead` 保留尾巴、把 `…` 放前面。

**`Opts` 欄拿掉了。** 它數的是一個區塊有幾行選項 —— 那是明細浮層講得清楚、而表格只能
暗示的事實,那幾格寬度花在路徑上更值得。代價是 `Host *` 那一列現在幾乎空著,而那是
誠實的:它真正設定的東西沒有任何一欄裝得下。

**明細的 section 標題是兩行,不是一行:**
...
原本是一行、中間一個 `·`,而它兩端都讀得不好:長路徑要嘛把 pattern 推出右邊(截斷從
尾巴切,所以被切掉的正是「這是哪個區塊」),要嘛讓眼睛在找路徑到哪裡結束。**疊起來
之後每一行是一件事,誰都擠不掉誰。**

**依檔案分段,三層縮排。**

> 「有辦法依照檔案整理嗎?同一個檔案的放同一個區塊?」
> 「這樣從 Host 開始就應該要縮排」「這樣他之下的都應該要縮排」

```
│ 󱁼 ~/.ssh/config                      │   ← 檔案
│   Host *                             │   ← 這個檔裡的區塊
│    AddKeysToAgent  yes               │   ← 那個區塊設了什麼
│    UseKeychain     yes               │
│                                      │
│   Host gw                            │
│    HostName        198.51.100.10     │
...
原本是**一個區塊一段**,所以同一個檔案的兩個區塊會把路徑印兩次。現在是**一個檔案
一段**,區塊縮在它底下、選項再縮一層。

**但只合併「連續的」同檔區段,不是真的按檔案收攏。** section 的順序**就是優先權**,
而 `Include` 寫在兩個 `Host` 中間時,ssh 讀的順序真的是 root → included → root。把
root 的兩塊收在一起,會把那件事整理成一個**關於誰先誰後的謊**。有測試釘住:那個交錯
情況要產生**三**段,不是兩段。

**級距不是等距的,而那是對的。** 這個浮層本來就把標題放在自己 label 左邊一格
(`Connection` 在 1、`Name` 在 2),所以把那一對嵌進檔案底下就是 2 然後 1 —— 是既有
慣例往下一層,不是新發明一種寫法。

`detailRow` 因此多一個 `depth`,而 **`labelW()` 必須把縮排算進去**:不算的話,最深的
那個 label 會剛好結束在它的值開始的地方,兩者黏在一起。這條的 mutation **一開始活
下來** —— 測試只斷言了三層有縮排,沒有斷言 label 跟值之間還留著空格。補了一條看
`AddKeysToAgent  yes` 中間那兩格的斷言。

**`[2] Config` 自己的 Enter 浮層也是同一個畫法。** 那裡的路徑本來就在,只是塞在
`Lines 2-5 of ~/.ssh/config.d/work` 這個值裡面 —— 是真的,但說在沒有人會先看的地方。
現在它是 section 標題,`Lines` 只剩行號:
...
第二行是一個 **label 為空的 row** —— 那正是 section 標題的畫法,所以兩行同縮排、同
`dimColor`、中間沒有空行。**顏色一個字都沒改**(使用者指定)。

`·` 換成 **`nf-md-file_cog_outline` `U+F107C`**:一格說完「一個設定檔」。用 outline
而不是實心的那一版,因為它站在一條**刻意在退場**的 dim 標題線上 —— 實心版帶的墨比
那條線該有的多。兩個 codepoint 都是從安裝的字型 cmap 讀出來的,不是憑記憶(§3.2.1)。

#### 設定很多的時候要捲得動

一個 `Host *` 帶三十個選項是很平常的事,所以這個浮層本來就要能捲 —— 而它**本來就
會**:`detailPopup.update` 直接呼叫 `moveScroll`,吃的是 `j`/`k`/`u`/`d`/`gg`/`G`,
跟其他清單同一套詞彙(§4.2)。`j/k scroll` 只在真的有溢出時才進 hint。

沒有寫程式,但**補了測試** —— 一個沒有測試釘住的既有行為,跟一個還沒做的功能在下次
重構時是同一件事。測試同時釘住**腳底的 offer 不會被捲走**:hint 承諾 Enter 有作用,
一個會捲出框外的承諾不是承諾(§11.29)。

#### 還是說不出來的部分

`Match` 條件 sshu 不判斷,`Include` 有可能**解析不到東西**(pattern 沒命中、檔案讀不
到、巢狀太深)。這些仍然掛在明細腳底的 `may also apply` —— 但現在那裡**只剩下真的
讀不到的**,跟得進去的都已經在上面用它們自己的檔名列著了。

`Unread` 與 `MatchConditions` 都**點名不計數**:一個「1 個 include 沒讀」的數字,會
讓人抱著自己的檔案找是哪一個。

#### 一個測試以外的收穫

改完之後編譯錯誤指出:測試 fixture 裡的 `Include ~/.ssh/config.d/*` 現在會**真的被
展開** —— 單元測試會去讀開發者自己家目錄的 config。全部改成**相對路徑**,那樣它就只
可能碰到 `t.TempDir()` 底下的東西。**一個之前無害的字串,在功能變強之後變成了越界。**

#### mutation 清單(14 個,全數被抓)

Include 完全不跟 / 被 include 的區塊接在最後而不是就地展開 / glob 不排序 / 相對路徑
用 cwd 解析 / 沒命中的 Include 默默吞掉 / 循環被跟下去 / 編輯寫進主檔而不是區塊自己的
檔 / 刪除從主檔刪 / `[A]` 寫進別的檔 / 每個檔案都重寫 / Include 之後的選項被孤立 /
狀態列不說有幾個檔 / 確認框說錯檔名 / 明細的 section 不再點名檔案。

其中「Include 之後的選項被孤立」第一次**沒被抓到,而那是我的 mutation 寫壞了** ——
`defer func(){ cur = -1 }()` 改的是一個離開函式時就丟掉的區域變數,不可觀察。換成真正
會孤立的寫法(遞迴回來後重設 `cur`)就被抓到了。**mutation 本身也會有 bug,而一個
「活下來」的結論在確認它真的改變了行為之前不算數。**
...
下一步已經定案(使用者的提案),見 §11.45:命令帶一個 hop 數,**每層減一,減到 0 的那層執行**。
...
### 11.45 由外層直接操作某一層 —— hop 定址

#### 使用者的提案

> 「如果要從最外層 broadcast 的時候可以加入一個 int, 然後一層減 1, 減到 0 就不要在傳,
> 這樣是否可以達成由外層直接對某一個內層發 broadcast? 0 的那一層就是指定的那一層,
> 這樣 popup 就只需要顯示在最外層?」

可以,而且這正是 TTL 的機制:**沒有任何一層需要全域視野**,每層只做「減一、轉發」,
收到 0 的就是目標。§11.44 讓最外層**看得到**整條鏈;這一節讓它**動得了**其中一層。

#### 我先前判斷錯的地方

§11.44 結尾寫過「Control 難很多,因為內層的 stdin 被 bubbletea 的按鍵解析器佔著,
拿不到原始 bytes」。**那個理由是錯的。** bubbletea 收 `tea.WithInput(io.Reader)`,
所以 sshu 可以在它之前先把私有序列從 stdin 抽掉、其餘原樣交出去 —— 跟 §11.44 的
`readLoop` 分流是**同一個形狀、反方向**。兩條通道對稱,不是一邊容易一邊難。

#### 定址慣例

| 選單第 N 列 | 送出的 hop | 誰執行 |
|---|---|---|
| 1(this sshu)| **不送** —— 就是上面那兩個字母 | 本層 |
| 2 | 0 | 下一層 |
| 3 | 1 | 下一層減成 0 轉發 → 第三層 |

收到命令的一層:`hop == 0` 就執行,否則**減一再往內送**。上限 16 —— 跟 OpenSSH 對
巢狀 `Include` 的上限同一個數字,理由也一樣:超過那個深度,數字不是鏈,是 bug。

#### ack 是免費的

不需要回覆協定。目標層狀態一變,**下一幀的 §11.44 報告就把新狀態帶回最外層**,那一列
自己會翻。所以列上顯示的永遠是「對面真的變了」,不是「我按了所以假設它變了」。按下去
就翻的 UI 會宣稱一件它還沒被告知的事。

#### 三件釘死的事

1. **lock 不吃命令。** locked 的語義是「所有**按鍵**穿透」,而命令不是按鍵 —— filter
   在 stdin 邊界、在按鍵處理之上。一個會吞掉定址的 lock,會讓 locked 那層底下的每一層
   都失聯,而那正是使用者 lock 之後所處的狀態。
2. **兩條通道用不同號碼**(報告 `7180`、命令 `7181`)。否則遠端的**輸出**可能被上一層
   讀成命令 —— 那等於讓對面驅動你的機器。有測試盯著這條。
3. **只對自己聽過的 hop 送命令。** 舊版 sshu 不懂這個序列,它的 bubbletea 會把那些
   bytes 當按鍵解析,可能在別人的機器上觸發隨機動作。§11.44 的報告帶版本號、而且鏈
   只到「會講話」的那層為止 —— 擋法就是那個。

#### 不扣住單獨的 ESC(以及它的代價)

filter 只在尾端**確實是前綴的開頭**時才扣住,而且**至少要兩個 bytes**。理由:使用者按
`Esc` 送出的就是單獨一個 ESC,扣住它等於讓**全 app 最常按的鍵**卡到下一個按鍵才生效
—— 為了一個他根本沒在用的功能。兩個 bytes 之後歧義就沒了(沒有人會打 ESC 再打 `]`)。

代價寫清楚:命令若剛好被核心切在第一與第二個 byte 之間,就不會被認出來。它是一次短
寫入、進到一個空閒的 PTY,實務上不會這樣切;而且**退化不是損壞** —— 每個 byte 仍原序
通過,最壞情況是幾個字元被打到遠端,不會有半執行的命令。有測試分別盯著「其他每個切點
都要成功」與「這個切點必須原樣通過」。

#### 只有實機跑得出來的那個 bug

`tea.WithInput` 包住 stdin 之後,**bubbletea 不再把終端機切進 raw mode**:

```go
// bubbletea/tty_unix.go:17
if f, ok := p.input.(term.File); ok && term.IsTerminal(f.Fd()) {
	p.previousTtyInputState, err = term.MakeRaw(p.ttyInput.Fd())
```

`term.File` 要 `io.ReadWriteCloser` + `Fd()`,而我的 filter 只有 `Read`。結果是整個
app 變成行緩衝、每個鍵都要等 Enter —— **形同壞掉,而測試全綠**。修法是讓 filter 把
`Fd`/`Write`/`Close` 轉給真正的 stdin;沒有檔案在後面時回一個不可能是終端機的 fd,讓
測試用的 `strings.Reader` 得到誠實的答案。回歸測試用的是**編譯期**的介面斷言,少任何
一個方法就編不過。

這是本輪最嚴重的缺陷,而它**只能靠跑真的 binary 抓到**。
...
> 「現在打開 alt+enter,內層還是會開啟 popup, 不會只有最外層」

本節原本就寫了「屆時 alpha 不再 broadcast,選單只開在最外層」,但**那段程式沒有真的
改**:§11.43 的無條件轉發原封不動留著,Control 疊上去之後兩者同時作用。是使用者實際
跑出來抓到的。

規則改成一句話:**alpha 永遠開本層的選單;要不要再轉發一份,看這一層能不能定址它裡面
的東西。**

| 那一格 | 行為 |
|---|---|
| **回報過**(會講話的 sshu) | **不轉發** —— 它已經是這個選單裡的一列,轉發只會在使用者本來就能操作的層上多疊一個 popup |
| **沒回報**(普通 shell、太舊的 sshu) | **照舊 broadcast**(§11.43)—— 那是進得去的唯一辦法 |

**混合鏈自動正確**,因為每一層問的是**自己那一格**:新→新→舊 的情況下,最外層直接
定址中間層,而中間層仍然對最舊的那層 broadcast。這條 fallback 不能省 —— 省掉就是讓
升級把還沒升級的人擱淺。

#### mutation:12 個全抓

hop 不減、命令在本層也照做、選單差一格定址、永遠只送 lock、選內層卻改到本層、內層不再
可選、畸形 verb 照樣執行、離譜的 hop 被接受、filter 不再放行一般 bytes、單獨 ESC 被扣住、
EOF 時 carry 被丟掉、報告被當成命令。

其中「filter 不再放行一般 bytes」第一次跑是 **build failure —— 那什麼都沒證明**
(`kept` 變成未使用變數),換成 `_ = kept` 才是真的行為變更,然後才被抓到。

### 11.46 把顏色深度帶過 ssh

#### 症狀

使用者發現巢狀時,內外層的 popup **顏色不一樣**,並據此推論「內層的選單其實畫在
root 上」。不是 z-order —— 同一支 binary、同一個 layer,差別只有一個環境變數:
...
**ssh 只自動轉送 `TERM`,不送 `COLORTERM`。** 所以遠端的 sshu 判定自己只有 256 色,
**整個 UI 的每一個顏色都被量化**,不只 popup。

#### 兩端都要動,而且都不能靠使用者的設定檔

- **送出端**:`sshEnv` 帶一個 `LC_SSHU_COLORTERM`,並且 `buildSSHCmd` 明確加
  `-o SendEnv=LC_SSHU_COLORTERM`。**不依賴使用者 ssh_config 裡有沒有
  `SendEnv LC_*`** —— 很多有、不是全部有,而一個「取決於沒人編輯過的設定檔」的顏色,
  就是會在某人機器上莫名其妙錯掉的顏色。
- **接收端**:`AdoptForwardedColor()` 在 `main()` 最前面、任何東西 render 之前執行。

`LC_` 這個前綴是**現成的走私通道**:sshd 預設的 `AcceptEnv` 就收 `LC_*`,iTerm2 的
`LC_TERMINAL` 走的也是這條。名字中間放 `SSHU`,免得被任何真的在讀 `LC_*` 當語系的東西
誤認。實測(`ssh localhost`)確認 macOS 的 sshd 預設就放行。

#### 只填空,不覆蓋

`AdoptForwardedColor` 只在 `COLORTERM` **是空的**時候才寫入。真的有 `COLORTERM` 表示
本機終端機自己講話了,它贏 —— 轉發來的值是上一層對**它自己**終端機的描述,不該蓋過
這一層對自己的。同理,本機沒有深度可講時**什麼都不帶**:送一個空值等於宣稱一個從來
沒被回報過的深度。
...
|---|---|---|
| **修正後** | 131 | **0** |
| 對照(把變數清掉,等同修正前) | 42(只剩外層自己的) | **110** |

對照組是必要的:只看「修正後全是真彩」無法排除「這個量測方法本來就看不到 256 色」。

#### mutation:5 個全抓

深度沒放進子行程環境、沒叫 ssh 送、空值也照送、轉發值蓋過本機的、遠端根本不採用。

### 11.47 zoom 的第三階:滿版 —— 讓「無限套層」在幾何上也成立
...
|---|---|---|
| 0 | — | 網格照排 |
| 1 | **zoompanel** | 焦點格佔滿 panel(網格區),chrome 與邊框都還在 |
| 2 | **zoommax** | 連 chrome 三列與邊框都不畫,整個顯示區都是那個終端機 |

程式裡是 `zoomPanel` / `zoomMax`,footer 說的是 `zoom panel` / `zoom max`,
命令通道的 verb 是 `zoommax` / `unzoom`。同一組詞從對話一路到線上,沒有第二套
說法 —— 這正是設計文件存在的理由之一。

#### 起因:機制成立,幾何不成立

v1.5.0 花了三套機制(per-session lock、OSC 7180 自我通報、OSC 7181 hop 定址,
§11.43–§11.45)讓巢狀 sshu 可用。但**每一層 sshu 都要 5 列**:`chromeRows`
3 列(tab 列、rule、footer)加上格子邊框上下 2 列。而 `View()` 在
`m.h < chromeRows+3` 就印 `terminal too small`,所以每層至少要 6 列。
...
**標準終端機上大約第 3 層就到頂。**一個宣稱能做到、實際做不到的能力,比沒有
這個能力更糟。這一節就是把那句話補完。

#### 三階段,而不是換掉原本的行為

使用者的提案:`Alt+Z` 走三站,回到原點。

```
正常 ──alt+z──▶ 佔滿 panel ──alt+z──▶ 滿版 ──alt+z──▶ 正常 ──▶(循環)
      (原本的 zoom)          (本節新增)
```

這比「把 zoom 直接改成滿版」好:原本的行為仍是一個到得了的狀態,滿版永遠是
使用者按了**兩次**才發生的事,不會撞到誰。

#### 空轉的那一階要跳過

照字面實作會壞在巢狀場景:**內層 sshu 通常只有一個 session**,而「一格佔滿
網格」在只有一格時什麼都不會變 —— 這正是 §11.25「`canZoom()` 為 false」原本
的理由,那個理由對**第一階**依然成立。於是循環會變成:

```
正常 → alt+z →(畫面完全沒變)→ alt+z → 滿版 → alt+z → 正常
```

按一下沒反應、再按一下才動。**一個有隱形站的循環,會被讀成壞掉的鍵。**

所以 `nextZoom()` 逐階問「這一階會不會改變畫面」,不成立就跳過:一格時是兩態,
多格時是三態。代價是**循環長度隨情境改變**,這件事必須寫在文件裡 —— 它是這個
設計唯一沒有消掉的妥協,只是比「每層付一列」便宜得多。

#### `Alt+Esc` 反向走同一個循環

`Alt+Esc` 從「清掉 zoom」改成「退一階」,而且退完了才收回鍵盤。這不是為了對稱
好看,而是它跟 §11.25 已確立的規則是同一條:**一次剝一層,層是使用者自己疊
上去的**。疊了兩階就該剝兩次。副作用是三階段最常被詬病的「衝過頭只能再繞
一圈」不存在了 —— `Alt+Z` 往前、`Alt+Esc` 往後。

跳過的規則兩邊必須一致,否則回程會走到去程沒有的站。實作上**只在一個地方
決定**:`zoomAt()` 會把「沒有第二格可讓」的網格階直接降級成 `zoomOff`,
`prevZoom()` 因此不重複判斷(見下面的 mutation 記錄)。

#### 滿版拿掉的是什麼

- **邊框**:`cellView` 的 `innerW/innerH` 從 `w-2, h-2` 變成 `w, h`,並且不套
  `panelChromeTone`。
- **chrome 三列**:`View()` 在滿版時**不組**那個 frame,直接回 `m.panel()` ——
  跟 splash 同一個形狀(整個 frame 被接管),那是既有前例。
- **幾何**:`gridArea()` 在滿版時回 `(m.w, m.h+chromeRows)`。面板拿到的高度是
  app 高度減 chrome,所以加回來**就是** app 高度 —— **推導,不儲存**。理由:
  階段改變不是一次 resize,不會經過 `setSize`,存一份副本會在每個不經過
  `setSize` 的轉換上過期。

#### 疊上去,不是留下來

滿版之後 sshu 沒有 chrome 可以說話,但**留一列**是錯的答案:留一列 = 每層一列,
線性稅原封不動搬回來,只是斜率變小。

所以滿版裡 sshu 自己的所有揭露都是 **overlay**:蓋掉輸出的幾個 cell,**不佔任何
一列**。而且它不會隨深度累積,因為滿版之後**每層拿到的幾何完全相同**,每層又都
畫在同一個座標,於是它們疊在彼此身上:

```
第2層畫  sshu      在右上角
第1層畫  sshu      在右上角   ← 蓋掉第2層的
第0層畫  sshu ×3   在右上角   ← 蓋掉第1層的
畫面上   sshu ×3              ← 只有一個,幾個 cell
```

深度 2 和深度 10 被遮住的是同樣那幾個 cell。**O(1)。**

> **不要讓每層錯開座標。**那看起來體貼(不遮到內層的 badge),實際上會累積成
> n 個 badge —— 正好變回這一節要消掉的東西。

#### badge:內容、位置,以及為什麼數字是對的

內容是 `sshu`、層數(>1 時)、以及這一層 locked 時的鎖頭 glyph。鎖頭必須在:
滿版之後,原本帶鎖頭的 title 和原本寫著 `alt+enter release` 的 footer 都不在
畫面上了。

層數前面是 **`nf-oct-arrow_down_right`(U+F43E)**,不是 `×`:後面那個數字數的是
**往內**有幾層,不是乘法。codepoint 是從安裝的 Nerd Font cmap 讀出來的,不是
憑記憶寫的 —— 這個專案的 glyph 名字已經記錯過三次。

顏色是 **lavender `#b4befe`**(使用者指定),與 `editColor` 同一個 hex。§B 禁止
借別的 band,這裡是**記錄下來的例外**而不是默默違反:兩者永遠不會同時出現在
同一個表面上 —— badge 只在 chrome 消失後存在,而編輯中的欄位畫在有自己邊框的
浮層裡。程式裡取名 `nestColor` 而不是直接用 `editColor`,讓這個共用是刻意且
grep 得到的。

**位置是右上角,不是右下角。**在 shell 裡最下面那一列是**正在輸入的那一行**,
蓋住它會讓打進去的字看不見;最上面那一列是已經捲過去的輸出。兩害相權。

**層數為什麼只有最外層數得出來,又為什麼剛好是它畫得贏**:通報是由內往外走的
(§11.44),每一層知道自己**裡面**有什麼、對外面一無所知。而繪製順序也是由內
往外 —— 內層的輸出經 ssh 進到外層的 vt10x,外層才把它畫進自己的格子。所以
**唯一知道整條鏈有多長的那一層,剛好就是畫在最後、蓋得過所有人的那一層**。
數字在它被讀到的地方永遠是對的。

#### 選取模式:legend 疊上去,而不是把邊框畫回來

選取模式(§11.33)的主要視覺訊號是**整圈邊框轉黃**,滿版之後沒有邊框。

**否決的做法:滿版時讓選取模式把邊框畫回來。**兩個實際問題:

1. 它會改變 `focusedCellSize()` —— 而那正是「凍結的那一頁被擷取、之後又被畫
   回去的那個框」。進入模式時可見高度會掉 2 列。
2. 更糟的是它會 `applyGeometry()` 重算並**對遠端發 SIGWINCH** —— 一個「遠端
   完全不參與」的模式,進出各讓遠端重畫一次。

**採用的做法**:把 footer 那份 legend 疊到**最後一列**。頁面在選取模式下本來
就是凍結的,蓋住一列不會失去任何會動的東西 —— 這跟「可以蓋它」是同一個理由。
而且它救回來的是 **legend 本身**(`y` / `v/V` / `hjkl` / `u/d` / `alt+v`),
那是 §A.1 的強制揭露,比那圈黃更要緊。

legend 兩邊共用 `copyLegendPairs()`:同一個模式在兩個地方用兩份清單揭露,
遲早有一份是錯的。

#### footer:三個標籤,而且排在 `alt+v` 後面

三站需要三個標籤,所以 footer 說的是**下一次按會做什麼**:`zoom` /
`full screen` / `unzoom`(滿版沒有 footer,所以最後一跳無標示,跟 `Alt+Esc`
一樣)。而且它現在**無條件提供** —— 永遠有 chrome 可拿。

改完當場踩到 §11.33:`full screen` 比 `zoom` 長,在 40 欄(這一列剛好放得下
兩組)把 `alt+v select` 擠掉了,而 §11.33 明文要求 select 必須撐過窄 footer。
修法是把 `alt+z` 排到 `alt+v` **之後**(`alt+enter lock` 仍然最後,它最罕用)。
這是測試抓到的,不是想到的。

#### 巢狀時,內層怎麼被 zoom

`Alt+Z` 現在被每一個**未鎖**的層攔下,所以內層收不到它。這不需要新機制:
**鎖住外層,和絃就往下走**(§11.43 的 locked 分支在 `Alt+Z` 之上)。要把第 2
層弄成滿版,就鎖住第 0、第 1 層,`Alt+Z` 自然抵達。

#### 一列選單:整條鏈一次到位

`Alt+Z` 只管自己這一層(跟其他所有和絃一致),所以深度 N 的滿版要**手動跑一圈**:

```
Alt+Z                    第 0 層 zoommax
Alt+Enter → Lock         鎖第 0 層(和絃才往下走)
Alt+Z                    第 1 層 zoommax
Alt+Enter → 選內層那列    從最外層鎖第 1 層(§11.45 的 hop 定址)
Alt+Z                    第 2 層 zoommax
```

**2N 步,而且順序不能錯** —— 鎖住的層自己就不能再 zoom(所有和絃都穿透過去
了),先鎖後 zoom 就卡住,得先 Release。

使用者的裁決是:**這一串本身是對的**。和絃遇到 lock 就穿透、遇到未鎖就生效,
是可預知、可操作的;「太自動反而奇怪」。要優化的不是規則,是那個 loop。
...
選它、而不是選「`Alt+Z` 自動往下傳播」的主要理由 —— 後者會憑空造出一個
「全鏈滿版但沒鎖」的狀態,而且會替使用者做一個破壞性決定(某層若有多格網格,
自動傳播等於替他收掉)。

**最內層刻意不鎖。**鎖了的話所有和絃都穿透到它的遠端 shell,於是在整片畫面
終於只屬於一個終端機的那一刻,sshu 的鍵一個都到不了正在畫它的那一層。實測:
深度 3、macro 跑完後按一次 `Alt+Esc`,它穿過兩個鎖住的層、由最內層接手退出
zoommax —— 剛好一層的 chrome 回來。

反向那一列**對所有層都送 release**,包括從沒被鎖的最內層。release 是幂等的,
而一個「必須記得自己跳過了誰」的還原,等於在維護第二份鏈狀態。

實作上沿用既有的 hop 通道,只多兩個 verb。**沒有任何順序要求**:命令與
SIGWINCH 誰先到都行,因為階段是**存**的、幾何是**推導**的,層拿到哪個尺寸就
用哪個尺寸算。

> 加 verb 時踩到一個既有地雷:`applyNestCmd` 原本寫 `want := Verb == lock`,
> 也就是**任何不是 lock 的 verb 都當成 release**。兩個 verb 時無害,第三個
> 一加就是靜默解鎖 —— zoom 命令會順手把那一層解開。改成 switch。

#### 接受的退步

- **深度計不見了。**今天那疊 chrome 很醜,但可以用眼睛數。滿版之後只能靠
  badge 的數字,而 badge 只有最外層畫得出正確的數字。**不修**:要讓每層都知道
  自己的深度,得讓命令通道反向帶一個深度戳記 —— 為一個「使用者自己走進去、
  每一步都是刻意的」場景新增協定,不划算。
- **傳輸進度條**在滿版期間看不到(它住在 tabRule)。zoom 隨時可退,Jobs 浮層還在。
- **最內層右上角那幾個 cell 的輸出永遠看不到。**固定成本,深度 10 跟深度 1
  付一樣多。

#### 否決的做法:把 `(sshu)` 注入遠端的 prompt

使用者提過:在遠端的 command prompt 前面加一段標記。否決,兩個理由:

1. **sshu 不擁有遠端的 prompt。**tab `[3]` 是 exec 真正的 `ssh`,PS1 來自遠端
   自己的 rc 檔。注入只有兩條路:當鍵盤輸入送 `export PS1=...`(會進 history、
   跟 shell 啟動賽跑、遠端不是 bash/zsh 就壞、一登入就進 TUI 或容器更壞),
   或要求遠端 rc 檔配合 —— 而「得先去設定遠端」正是 sshu 存在要免掉的事。
2. **決定性的:它只在 shell prompt 前有效。**一開 vim / htop / less,prompt 就
   不在畫面上 —— 而那正是要滿版的原因。**指示器會在最需要它的時候消失。**

overlay 沒有這兩個問題:sshu 畫在最後,全螢幕程式蓋不掉它。

#### mutation:14 個全抓,2 個存活各自處理

抓到的:`nextZoom` 不跳過空轉階、回程不落在網格階、滿版仍付邊框、`gridArea`
不把 chrome 加回來、`cellView` 照畫邊框、陳舊的網格階活過網格縮小、badge 不數、
badge 掉鎖頭、overlay 不守列寬、選取模式失去 legend、`unzoomOne` 一次剝光、
滿版仍組 frame、footer 用固定標籤、`Alt+Z` 又轉發給遠端。

加上這一節後半(macro、badge、顏色)的第二輪,共 **23 個 mutation 全抓**。

三個存活的**不是補測試了事**:

- **`prevZoom` 的跳過判斷**:`zoomAt()` 已經把「沒有第二格」的網格階降級了,
  所以 `prevZoom` 再判斷一次**永遠觀察不到**。這種存活是**要刪的程式碼**,不是
  要補的測試 —— 同一件事在兩個地方決定,是它們日後走散的方式。
- **`zoomAt()` 的降級**:反過來,它是真的沒被測到 —— 整組測試沒有任何一個走到
  「格子在 zoom 底下離開網格」。可達路徑是**另一格的 session 結束**
  (`reap()` 保留鍵盤,multipty_test 已釘住)。補了 `TestAGridZoomDoesNotOutliveItsGrid`:
  剩一格時網格階必須是隱形的,否則 `Alt+Esc` 會花掉一次按鍵去離開一個畫面上
  不存在的階段。
- **`setZoom` 裡的 `inCell` 檢查**:我為它寫的測試**空過了** —— 焦點不在格子時
  `currentSession()` 就回 nil,`applyNestCmd` 在那裡就返回,根本沒走到
  `setZoom`。是 mutation 說出來的,不是我看出來的。那道檢查因此永遠觀察不到
  (每個呼叫端都已經確立了格子),刪掉;測試改成釘真正生效的那道守衛,並用
  一個停用該守衛的 mutation 證明它不再空過。
